...
function waitForMsgType<Data extends { type: string } = { type: string }>(
  target: AbstractWorker | Window,
  type: Data["type"],
): Promise<Data> {
  return new Promise<Data>((resolve) => {
    target.addEventListener("message", function onMsg({ data }: Event & { data?: any }) {
...
// @internal
export function exitMeasure(exitFn: () => void) {
  exitFn()
}
//#endregion utils
...
// @internal
export async function createWorkerInitMessage(
  module: WebAssembly.Module,
  memory: WebAssembly.Memory,
  builder: WasmBindgenRayonPoolBuilder,
) {
